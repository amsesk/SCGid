...
class RSCUTree(object):
    ...
    def annotate(self):
        ...
        if self.mode == "blastp":
            ...
            raise NotImplementedError(
                "Annotation with NCBI nt blast results not yet implemented.")
    ...
    def pick_clade(self, error_catch=True):
        ...
        # However, based on how it's written now, if a monophyletic clade is
        # nested within another, some (likely) poorer quality clades will
        # include both and lead to a node ending up in multiple bins, BUT
        # hopefully selection of the best clade from each bin sorts this out
        bins = {}
        ...
        # Now compare the best trees from each bin to decide on a best tree (or
        # set of best trees) for subsequent actions
        ...
        if len(best_trees) > 1:
            ...
            if len(best_trees) > 1:
                self.head.logger.warning(
                    "More than one best tree detemined from codon analysis.")
            else:
...
class Codons(Module, LoggingEntity, Head, ErrorHandler):
    ...
    def __init__(self, argdict=None, loglevel=logging.INFO, hollow = False):
        ...
        if argdict is not None:
            self.translated_args = self.translate_argdict(
                argdict, Codons.generate_argparser())
            ...
            # Copy command line args defined by self.argparser to self.config
                ...
                if spl[2] == "CDS":
                    ...
                    if shortname in contig_chunks:
                        if pid in contig_chunks[shortname]:
                            contig_chunks[shortname][pid].append(
    ...
    def concatenate_cds(self, contig_chunks, nucl):
        ...
        # pull CDS sequences from nucleotide fasta
        for shortname, pids in contig_chunks.items():
            ...
            if len(contig_cds_cat) != 0:
                cds_concatenates[shortname] = CDSConcatenate(
                    shortname, contig_cds_cat)

        # Return all contig-level CDS concatenates as a DNASequenceCollection
        # object
        return DNASequenceCollection().from_dict(cds_concatenates)

    # Function that confirms size of CDSConcatenate DNASequenceCollection is
    # larger than minlen arg
    @check_result
    ...
    def run(self) -> DNASequenceCollection:
        ...
        # Annotate tips of the dendrogram. Mode is handled inside function
        # body.
        nj_tree.annotate()
        ...
        # comparing clades' target:nontarget ratios
        nj_tree.pick_clade()

        # Return filtered DNASequenceCollection with non-trainset contigs
        # excluded
